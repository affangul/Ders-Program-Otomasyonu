...
{
    public partial class Derslikreport : Form
    {
        public Derslikreport()
        {
            InitializeComponent();
            derslikcombo(); // form ilk combobox doldurma metodu çalıştırılıyor
        }
        ...
        private void report()
        {
            string secilenDerslik = comboBox1.SelectedValue.ToString();  // comboboxdan seçilen bilginin kod kısmında atanmış değeri secilenDerslik ye tanımlanır
            
            Program.bag.Open();
            string sql = "SELECT Gün, Ders, Sınıf, Öğretmen, Başlangıç, Bitiş FROM Final " +
                          "WHERE Derslik = @prm1 " +
        ...
        private void ekbilgi() // word veya pdf çevirilirken çevrilen dersliğin ismini göstermek için metot
        {
            string derslikisim = "Derslik: " + comboBox1.Text; // ismi combobox1 den çekili
            ReportParameterCollection rprm = new ReportParameterCollection(); //parameterları tanımlamak için değişken tanımlandı
            rprm.Add(new ReportParameter("derslikisim", derslikisim)); //daha önceden oluşturulmuş parameter ismi ile eklenecek veri eşleştirildi
        ...
        private void derslikcombo() // combobox1 de derslik ismini göstermek için metot
        {
            ...
            comboBox1.DisplayMember = "Derslik_İ_İsim"; // görünecek olan bilgi
            comboBox1.ValueMember = "Derslik_İ_İsim"; //kod kısmında işlenen bilgi
            comboBox1.DataSource = dt;  // dt üzerinde olan bilgi comboboxa aktarılır
        }

        private void Derslikreport_Load(object sender, EventArgs e)
        {
